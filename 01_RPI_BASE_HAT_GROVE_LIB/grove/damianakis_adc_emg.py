...
# HOW TO RUN EMG
# works ok, except for grace, which seems to work only statically (despite the redraw)
# mkfifo fifo
# xmgrace -npipe fifo  &
# python damianakis_adc_emg.py > /tmp/1 &
# while [ 1 ] ; do echo "read\"/tmp/1\"" > fifo; echo autoscale > fifo ; echo redraw > fifo; sleep 1;done
#                       tail -f /tmp/1; -> to test, but not in production
#
#
#contrast with working example
# mkfifo fifo
# xmgrace -npipe fifo  &
# c=0; while [ 1 ] ; do let c=$c+1; echo $c >> /tmp/1; sleep 0.1; done &
# while [ 1 ] ; do echo "read\"/tmp/1\"" > fifo; echo autoscale > fifo ; echo redraw > fifo; sleep 1; done
#                       tail -f /tmp/1; -> to test, but not in production
